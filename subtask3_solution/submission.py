"""
Generate Submission Files for SemEval 2026 Task 9 - Subtask 3
...
# Label columns for subtask 3 (multi-label)
LABEL_COLUMNS = [
...
class MultiLabelPredictor:
    """Predictor for multi-label sequence classification models (subtask 3)"""
    def __init__(self, model_path: str, device: str = None, threshold: float = 0.5):
        ...
        # Try to load tokenizer/model robustly (local or remote)
        try:
        ...
        try:
            ...
            self.model = AutoModelForSequenceClassification.from_pretrained(str(self.model_path), trust_remote_code=True)
...
def generate_predictions_for_language(predictor, test_dir: Path, language: str, output_dir: Path):
    ...
    # predictions: list of [0/1,...] for each label
...
def create_submission_package(output_dir: Path, languages: list = None):
    subtask_dir = output_dir / "subtask_3"
    ...
    zip_path = output_dir / "subtask_3_submission.zip"
    with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for pred_file in subtask_dir.glob("pred_*.csv"):
            arcname = f"subtask_3/{pred_file.name}"
...
def main():
    parser = argparse.ArgumentParser(
        description='Generate submission files for SemEval 2026 Task 9 - Subtask 3'
    )
    parser.add_argument('--model_path', type=str, required=True, help='Path to trained model directory')
    parser.add_argument('--test_dir', type=str, default='./dev_phase/subtask3/dev', help='Directory containing test CSV files (one per language)')
    ...
    if not test_dir.exists():
        ...
        return
    logger.info("="*60)
    logger.info("SemEval 2026 Task 9 - Subtask 3 Submission Generator")
    logger.info("="*60)
